...
public class SearchSchedule_S {
  ...
  public void search_s_menu(String id) {
    ...
    searchLoop : while (true) {
      try {
        ...
        System.out.println("1.제목으로 검색  2.날짜로 검색  3.내용으로 검색 4. 공유자로 검색  0.뒤로가기");
        ...
        switch(menuNum) {
          ...
          case 4 : this.search_s_shares(id); break;
  ...
  public void search_s_shares(String id) {
    banner.printbanner2("공유자로 검색");
    try {
      Boot boot = new Boot();
      Connection CN = boot.boot();
      Statement ST = CN.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
      ...
      if (RS.next() == false) {
        ...
        while (true) {
          ...
          String keyword = sc.nextLine();
          if (keyword.equals("0")) {return;}
          msg = "select * from s_seet_"+id+" where shares = '"+keyword+"'";
          RS = ST.executeQuery(msg);
          RS.last();
          int searchResultCount = RS.getRow();
          RS.beforeFirst();
          if(searchResultCount == 0) {
            ...
            System.out.printf(" %s 님과 공유된 일정 총 %d 건의 검색 결과가 있습니다.\n\n",keyword, searchResultCount);
            showtable.searchResultShowTable_S(id, "select rownum, a.s_date, a.title, a.contents, a.shares, a.s_id from (select * from s_seet_"+id+" order by s_date) a where shares = '"+keyword+"'");
          }
        } // search While end
      } // RS check End
    } catch (Exception e) {System.out.println(e);} // try-catch End
  } // search_shares Method End
} // Class End
